...
/**
 * Интерфейсы BeforeEachCallback и AfterEachCallback позволяющие выполнить что-либо до и после каждого тестового метода.
 * Данный класс реализован для "изящного" прокидывания драйвера в тесты.
 * Для каждого теста реализована своя сессия драйвера: перед выполнением теста метод beforeEach открывает драйвер,
 * после выполнения теста метод afterEach его закрывает.
 */
